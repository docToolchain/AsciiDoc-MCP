"""
AsciiDoc document processor for analyzing and extracting information from AsciiDoc.
"""
...
class AsciiDocProcessor:
    ...
    async def analyze_document_structure(
        self, file_path: str, include_content: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for match in self.heading_pattern.finditer(content):
                ...
                if include_content:
                    ...
                    for next_match in self.heading_pattern.finditer(
    ...
    async def find_includes(
        ...
        try:
            ...
            def find_includes_in_file(
                current_path: Path, depth: int = 0
            ) -> List[Dict[str, Any]]:
                ...
                try:
                    ...
                    for match in self.include_pattern.finditer(content):
                        ...
                        if not Path(include_path).is_absolute():
                            resolved_path = (
                                current_path.parent / include_path
                            ).resolve()
                        else:
                        ...
                        if (
                            recursive and resolved_path.exists() and depth < 10
                        ):  # Prevent infinite recursion
                            nested_includes = find_includes_in_file(
    ...
    async def search_content(
        self, query: str, file_path: Optional[str] = None, case_sensitive: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        Args:
            ...
            file_path: Optional path to specific file, searches current directory
                if not provided
                    ...
                    for i, line in enumerate(lines):
                        if search_pattern.search(line):
                            ...
                            result = {
                                ...
                                "match_positions": [
                                    m.span() for m in search_pattern.finditer(line)
                                ],
                            }
